...
void selectTournament(int dif,int riv)
{
	if(dif==1)
	{
...
void startFight(Guardian* headRef,Guardian* seleccionado,Guardian* newGuardian,Guardian* player,Guardian* oponente,int op,int riv,int dif)
{
	char nombre_riv[50];
	//int op=0;
	int dado=0;
...
void menuOptions()
{
		...
		else if(k==3)
		{
			...
			scanf("%d",&dificultad);
			selectTournament(dificultad,rivales);
		}
		else if(k==4)
		{
			printf("----------------GUARDIANES---------------------\n");
		...
		{
			startFight(headRef,seleccion,newGuardian,jugador,oponente,op,rivales,dificultad);	
		}
		else if(k==6)
		{
			dado = getRollResult();
